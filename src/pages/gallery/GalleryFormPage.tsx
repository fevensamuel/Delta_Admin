...
import { getGalleryItems, createGalleryItem, updateGalleryItem } from '../../api/gallery';
...
export const GalleryFormPage: React.FC = () => {
  ...
  const loadExistingItem = async (itemId: string) => {
    ...
    try {
      ...
      const found = items.find((g) => g.id === itemId);
      if (found) {
        setItemToEdit(found);
  ...
  // Helper to calculate video duration from file or URL
  const calculateVideoDuration = (videoObjectUrl: string) => {
  ...
  // Process video file selection from device
  const handleVideoFileSelect = (file: File) => {
    if (!file.type.startsWith('video/')) {
      showToast('error', 'Please select a valid video file (MP4, WEBM)');
      return;
    }

    const objectUrl = URL.createObjectURL(file);
    calculateVideoDuration(objectUrl);

    const reader = new FileReader();
    reader.onload = (e) => {
      if (e.target?.result) {
        setVideoUrl(e.target.result as string);
        showToast('success', 'Video file loaded from device');
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (id) {
        ...
        showToast('success', 'New gallery item created successfully');
      }
      navigate('/gallery');
    } catch {
    ...
    <RoleGuard module="gallery" action={id ? 'edit' : 'create'}>
      <div className="max-w-3xl mx-auto space-y-6 pb-12 animate-in fade-in">
        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg border border-[#E2E8F0] shadow-xs space-y-6">
          ...
          {/* Device Image Thumbnail Upload Area */}
          <div>
              ...
              <div className="flex items-center gap-3 pt-2">
                <div className="flex-1">
                  <label className="block text-xs font-bold text-[#111827] mb-1 flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5 text-[#C8102E]" /> Video Duration (Auto-Calculated)
                  </label>
                  <input
                    type="text"
                    value={duration}
                    onChange={(e) => setDuration(e.target.value)}
                    placeholder="e.g. 02:45 (Calculated on file select)"
