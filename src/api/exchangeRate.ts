...
export const getExchangeRate = async (): Promise<number> => {
  try {
    ...
    return response.data?.data?.rate || response.data?.rate || Number(import.meta.env.VITE_EXCHANGE_RATE_FALLBACK || 159.98);
  } catch {
    // Return default fallback rate if server endpoint fails
    return Number(import.meta.env.VITE_EXCHANGE_RATE_FALLBACK || 159.98);
  }
};
